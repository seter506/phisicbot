...
async def get_unconf_clients(message: Message):
    ...
    for i in cl:
        inl_ok = InlineKeyboardButton(text='Подтвердить', callback_data=f'call_ok_{i[0]}')
        inl_can = InlineKeyboardButton(text='Отменить', callback_data=f'call_cancel_{i[0]}')
...
def register_admin(dp: Dispatcher):
    ...
    dp.callback_query.register(client_confirm, F.data.startswith('call_'))